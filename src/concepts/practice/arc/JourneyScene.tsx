...
const COLORS = ['#d6dfbd', '#aec5a0', '#bad3c7', '#e0b58f', '#d8d0a5', '#dfdec3'];
const morphVertex = `
...
const pointFragment = `
  ...
  void main() {
    ...
    float alpha = mask * vAlpha * (.28 + vLight * .7);
    if (alpha < .015) discard;
    vec3 ink = mix(uColor * (.68 + vLight * .38), vec3(.94,.72,.43), vWarm);
...
const lineFragment = `
  ...
  void main() {
    ...
    vec3 ink = mix(uColor, vec3(.91,.73,.48), signal * .7);
...
function WebGLJourney({ onReady, onError }: Props) {
  ...
  useEffect(() => {
    ...
    renderer.setClearColor(0x10201a, 0);
    ...
    try {
      ...
      const dustGeometry = new THREE.BufferGeometry(); resources.push(dustGeometry);
      const dustPositions = Float32Array.from({ length: 320 * 3 }, (_, i) => (hash(i + 12200) - .5) * (i % 3 === 2 ? 4 : 5.5));
      dustGeometry.setAttribute('position', new THREE.BufferAttribute(dustPositions, 3));
      const dustMaterial = new THREE.PointsMaterial({ color: '#c5d7b9', size: .009, transparent: true, opacity: .3, depthWrite: false });
...
/** Keep the original canvas renderer as a complete fallback for devices without WebGL. */
export default function JourneyScene(props: Props) {
  const [fallback, setFallback] = useState(false);
  const useFallback = useCallback(() => setFallback(true), []);
  return fallback ? <AsciiScene {...props}/> : <WebGLJourney onReady={props.onReady} onError={useFallback}/>;
}
