...
/** The hills are always present. Only the fireflies need a canvas or an animation clock. */
const DuskAtmosphere = forwardRef<DuskHandle, Props>(function DuskAtmosphere({ active, clocked }, ref) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      sprites.forEach(sprite => { sprite.width = 0; });
    };
  }, [active, clocked]);
    ...
    <svg className="arc-dusk-landscape" viewBox="0 0 1600 900" preserveAspectRatio="none">
      <defs>
        ...
        <radialGradient id={`${id}-afterglow`} cx=".76" cy=".49" r=".55">
          <stop stopColor="#f5bfbc" stopOpacity=".5"/><stop offset="1" stopColor="#eab1bf" stopOpacity="0"/>
        </radialGradient>
        <linearGradient id={`${id}-far`} x2="0" y2="1"><stop stopColor="#a28ead"/><stop offset="1" stopColor="#716b93"/></linearGradient>
        <linearGradient id={`${id}-middle`} x2="0" y2="1"><stop stopColor="#7c759d"/><stop offset="1" stopColor="#4b537c"/></linearGradient>
        <linearGradient id={`${id}-near`} x2="1" y2=".4"><stop stopColor="#3f405f"/><stop offset="1" stopColor="#535979"/></linearGradient>
        <linearGradient id={`${id}-foreground`} x2=".7" y2="1"><stop stopColor="#393550"/><stop offset="1" stopColor="#242b45"/></linearGradient>
      </defs>
      <path fill={`url(#${id}-sky)`} d="M0 0H1600V900H0Z"/>
      <path fill={`url(#${id}-afterglow)`} d="M0 0H1600V900H0Z"/>
      <path fill={`url(#${id}-far)`} d="M0 514C155 496 243 395 398 418S675 533 817 474 1001 364 1170 432 1429 468 1600 398V900H0Z"/>
      <path fill={`url(#${id}-middle)`} d="M0 580C167 457 262 494 402 557S639 632 822 544 1096 495 1234 552 1441 594 1600 507V900H0Z"/>
      <path fill={`url(#${id}-near)`} d="M0 660C184 695 307 622 451 611S713 698 886 648 1173 565 1320 616 1502 695 1600 643V900H0Z"/>
      <path fill={`url(#${id}-foreground)`} d="M0 714C218 647 344 739 534 779S828 730 1020 757 1371 794 1600 726V900H0Z"/>
    </svg>
