...
export function projectSource(p: PortfolioProject): ChatSource {
  return { id: p.id, kind: 'project', title: p.title, label: p.client, url: `/#case=${p.id}`, image: cover(p), content: `${p.description}\nWill’s contribution: ${p.involvement || 'Not documented.'}\nYear: ${p.year || 'Not documented.'}` };
}
export function articleSources(articles: PortfolioArticle[]): ChatSource[] {
  ...
  const records: ChatSource[] = articles.map(a => {
    ...
    return { id: a.id, kind: 'article', title: a.title, label: publication ? 'Essay · Medium' : 'Writing · Will Dzierson', url: publication?.url || `/#essay=${a.id}`, image: a.image_url, content: a.content };
  });
