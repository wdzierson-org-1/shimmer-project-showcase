...
import type { ConversationMessage } from './willbotPrompt';
import '@xterm/xterm/css/xterm.css';
import {
  prepareTerminalParagraphLine,
  spansToLineLinks,
  wrapTextWithOffsets,
} from './crHeroTerminalLinks';

const TERMINAL_STATE_KEY = 'portfolioOriginalCrtState';
const TERMINAL_META_KEY = 'portfolioOriginalCrtMeta';
const TERMINAL_LINK_HINT_KEY = 'crtLinkHintSeen';

const sleep = (ms: number) => new Promise<void>((r) => setTimeout(r, matchMedia('(prefers-reduced-motion: reduce)').matches ? 0 : ms));
...
const CRTHero = ({ onNavigate }: { onNavigate: (href: string) => void }) => {
  ...
  useEffect(() => {
    ...
    const applyLayout = (w: number, h: number) => {
      ...
      const rawScale = Math.max(w / BASE_W, w < 600 ? 0.5 : 0.65);
    ...
    (async () => {
      ...
      const crtSettings = {
        ...
        brightness: 0.58,
        ambientLight: 0.06,
        flickering: reducedMotion ? 0 : 0.06,
        horizontalSync: 0.005,
        jitter: reducedMotion ? 0 : 0.01,
        staticNoise: 0.02,
        glowingLine: reducedMotion ? 0 : 0.06,
        burnIn: reducedMotion ? 0 : 0.25,
        rasterizationMode: 1,
        rasterizationIntensity: 0.22,
        rgbShift: 0,
      };
      crt = new CRTTerminal(crtSettings);
      crtRef.current = crt;
      crt.attachXTerm(xterm);

      // ── Resize handling ───────────────────────────────────────────────────
      let lastWidth = wrapper.clientWidth, lastHeight = wrapper.clientHeight;
      const handleResize = () => {
        if (lastWidth === wrapper.clientWidth && lastHeight === wrapper.clientHeight) return;
        lastWidth = wrapper.clientWidth; lastHeight = wrapper.clientHeight;
        applyLayout(lastWidth, lastHeight);
        // Reallocate the library's canvas textures on a window-size change.
        // Keep xterm (input, transcript and history) alive across GPU recreation.
        crt?.detachXTerm();
        crt?.dispose();
        crt = new CRTTerminal(crtSettings);
        crt.attachXTerm(xterm);
  ...
  return (
    ...
    <div
      className="original-crt"
      style={{ position: 'relative', width: '100%', height: '100%', background: '#111118' }}
    >
